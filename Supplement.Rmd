...
Additional file 1 shows the distribution of some key characteristics in the children sampled as controls into the nested case control cohort and in the overall cohort at the 2-month visit. 
...
colnames(additional_file1)[5:6]=c('Not in nested case control sample N=905', 
 ...
 write.xlsx(additional_file1, file.path(supp.out, 'AdditionalFile1.xlsx'), overwrite=T)
  
```



```{r compare wgs subset to nested group, eval=F}

MetaVisit %>% 
  filter(Visit %in% c(3,5, 7))%>%
  select(Site, BabySex, BabyRace, Delivery, Education_HS, SalivapH, 
         Prim_d1ft, Prim_Tot_Teeth_Present, Case,
              ...
              .tbl_fun = ~.x %>% 
               tbl_summary(by=person_in_wgs,
                           ...
                           label = list(BabySex~"Child's sex", 
                                        ...
                                        BFCurrent~'Currently breastfed'),
                           statistic = list(all_continuous() ~ "{median} ({min}, {max})"))%>%add_p())
...
MV%>% 
  ...
  select(Site, BabySex, BabyRace, Delivery, Education_HS, SalivapH, 
         Prim_d1ft, Prim_Tot_Teeth_Present, Case,
         antibiotics_within_3mos_of_visit, IncidentVisit.f, Visit,
         person_in_wgs, BFCurrent)%>%unique()%>%
  tbl_strata(strata=Visit, 
              .tbl_fun = ~.x %>% 
               tbl_summary(by=Case,
                           ...
                           label = list(BabySex~"Child's sex", 
                                        ...
                                        Prim_Tot_Teeth_Present~'Count of primary teeth erupted/present', 
                                        Prim_d1ft~'Count of primary teeth with carious lesion, white spot, or filling', 
                                        IncidentVisit.f~'Visit of case diagnosis',
                                        antibiotics_within_3mos_of_visit~'Maternal report of child antibiotics within 3 mos prior to visit',
                                        BFCurrent~'Currently breastfed'),
                           statistic = list(all_continuous() ~ "{median} ({min}, {max})")))
...
```{r streptococcus over time, eval=F}
ps_long %>% 
  left_join(hellinger_network$intraStats)%>%
  left_join(hellinger_network$networkNames)%>%
  filter(OTU%in%c('ASV8', 'ASV10', 'ASV1', 'ASV14', 'ASV82', 'ASV26'))%>%
  ggplot(aes(x=AgeAtExamMonths, y=Abundance, color=Case, group=BabySubjectID))+
  geom_line(alpha=0.2)+
  geom_smooth(aes(group=Case))+
  theme_bw()+
  facet_wrap(~module_label3+ScientificName, scales='free')
```
...
names(res_list)=paste0(visit_labels2[names(visit_labels2)%in% visits])
write.xlsx(res_list, file=file.path(supp.out, 'AdditionalFile6.xlsx'), overwrite=T, rowNames=T)
```
...
write.xlsx(module_membership, file=file.path(supp.out, 'AdditionalFile7.xlsx'), overwrite = T)
```
...
```{r TABLE 8}
write.xlsx(compare_cluster_table, file=file.path(supp.out, 'AdditionalFile8.xlsx'), overwrite = T)
```

## Replication analysis 

Supplemental figure 11 shows the full network modules from the Holgerson et al. cohort which were similar in composition to the ECC-associated networks in the COHRA2 cohort. Unlike in Main Figure 4, all of the edges are shown (in Main Figure 4C, edges with weights<0.03 are filtered out for visualization purposes). Notably, the protective *Haemophilus-Neisseria* network module had more nodes in the Holgerson et al. cohort than in the COHRA2 nested case-control cohort. The Holgerson et al. cohort had a much lower prevalence of ECC cases (10%) than the nested case-control cohort from COHRA2 (~50%) as a result of the design. Figure 12 shows other network modules from the Holgerson et al. cohort.  
...
```{r FIG 15, fig.cap='Relationships between unsupervised clusters, salivary pH and future S. mutans detection', fig.height=12, fig.width=25, eval=F}
ph_smut
...
smut_lag_cst<-netcst.lag %>% filter(moduleColor=='brown')%>%group_by(Visit, CST.lag)%>%mutate(n_total=n())%>%group_by(Visit, CST.lag, Prevalence_Streptococcus_mutans)%>%mutate(n=n())%>%mutate(percent=round((n/n_total)*100, 1))%>%select(n, n_total, percent)%>%unique()
```
...
```{r TABLE 9}
write.xlsx(t3_table, file=file.path(supp.out, 'AdditionalFile9.xlsx'), overwrite=T)
```

```{r FIG 17, fig.cap='Volcano plot taxa differentially abundant between cases and controls in whole genome shotgun sequenced plaque and saliva samples', fig.width=8, fig.height=11, eval=F}
taxa_volc
...
write.xlsx(r3_table, file=file.path(supp.out, 'AdditionalFile10.xlsx'), overwrite = T)
```
...
```{r Table 12}
write.xlsx(op_table, file=file.path(supp.out, 'AdditionalFile12.xlsx'), overwrite = T)
```
...
write.xlsx(enrichment_table, file.path(supp.out, 'AdditionalFile11.xlsx'), overwrite=T)
