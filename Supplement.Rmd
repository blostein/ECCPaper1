...
Cases and controls did not differ in the distribution of sex, site, or race (Additional file 1). 

All available saliva samples from the cases and controls from the visit in which they were selected and all preceding visits were pulled for 16S rRNA amplicon sequencing. Selected individuals occasionally missed visits, did not have a saliva sample available, or had a saliva sample which failed 16S amplicon quality control (Additional file 3). Additionally, we randomly selected a subcohort of 15 cases presenting with enamel lesions at or after the 36-month visit and 15 corresponding controls. Incident-visit plaque and saliva samples from these 30 individuals were submitted for whole genome shotgun metagenomic sequencing.

```{r TABLE 1}
MV=MV%>%mutate(IncidentVisit.f=factor(MV$IncidentVisit2, labels=visit_labels2[4:8]))
babyCharacteristics=createTable(compareGroups(Case~.-Case-UniquePersonID, data=MV %>% select(UniquePersonID, BabySex, BabyRace, Site, Case, IncidentVisit.f)%>%unique()))
export2xls(babyCharacteristics, file.path(supp.out, 'AdditionalFile1.xlsx'), which.table="descr", nmax=TRUE, header.labels=c())
```


```{r TABLE 2, include=FALSE}
source(file.path(code.dir, 'Analysis', 'sampleLoss.R'))
write.xlsx(lostSamples, file.path(supp.out, 'AdditionalFile3.xlsx'), overwrite=T)
```

```{r FIG 1, fig.width=13.2, fig.height=8, include=FALSE, eval=F}
...
write.xlsx(strep_blast %>% split(.$asv),
           file.path(supp.out, 'AdditionalFile4.xlsx'), overwrite=T)
```
...
ggsave(strep_cors, file=file.path(supp.out, 'SupplementalFigure3.eps'), width=8, height=10)
strep_cors
```
...
write.xlsx(res_list, file=file.path(supp.out, 'AdditionalFile5.xlsx'), overwrite=T, rowNames=T)
...
write.xlsx(module_membership, file=file.path(supp.out, 'AdditionalFile6.xlsx'), overwrite = T)
```


Using weighted co-occurrence network analysis, we identified 5 network modules of co-occurring ASVs. These networks were named after the top two most abundant taxa in the network, and the most central taxa in the network. The membership of all of the networks is in Additional file 6. To summarize these network modules, we summed the relative abundances of each taxa belonging to the network module compared the trajectories of these abundances over time between cases and controls. Change in the summed relative abundance of the 3 network modules not displayed in Figure 2B are shown in Supplemental Figure 6; the structure of the overall network is shown in Supplemental Figure 7. In Supplemental Figure 7, ASVs are nodes in the network. ASVs are colored by the module to which they were assigned and sized according to their abundance. The shape of the ASV node indicates if that ASV is more abundant in ECC cases (triangles) or controls (squares). Edges with a weight<0.1 have been eliminated from the network visualization for readability, otherwise edge width corresponds to the strength of the correlation between nodes. ASVs of special interest are labeled.  

Cases and controls differed in the summed relative abundance of these network modules and the distribution of community state types in both pre-incident (Table 1) and incident (Additional file 5) visits
...
Supplemental Figure 8 shows the overlap between the top two most abundant ASVs in each of the five network modules (ten ASVs) and the top ten most important ASVs for the separation of the community state types. There is significant overlap in these ASVs, with nine of the ten ASVs being the same. 
...
venn_comp=ggvenn(list('CST \n separation taxa'=top_seps, 'Abundant \n network taxa'=top_nets), show_elements = T, text_size=1.4, label_sep='\n')
```

```{r FIG 9, fig.cap='Venn diagram of top two ASVs in each of five network modules (10 ASVs) vs the top ten ASVs contributing to the separation of the community state types', fig.width=13}
venn_comp
...
For the Dirichlet multinomial community state typing, we varied the choice of k community state types. When decreasing k to 5, the case-associated 12-month community state type was combined with one of the 2-month community state types (Supplemental Figure 9). When decreasing k to 4, the control-associated community state type was also collapsed. However, case- and control-associated community state types apparent at and after the 24-month visit were still apparent. The clustering of the samples through community state types was overall fairly robust, according to the rand index, the adjusted rand index and the adjusted mutual information index (Additional file 7).
...
```{r TABLE 8}
write.xlsx(compare_cluster_table, file=file.path(supp.out, 'AdditionalFile7.xlsx'), overwrite = T)
```

## Replication analysis 

Supplemental figure 11 shows the network modules from the Holgerson et al. cohort which were similar in composition to the ECC-associated networks in the COHRA2 cohort. Figure 12 shows other network modules from the Holgerson et al. cohort.
...
Salivary pH alone did not distinguish cases and controls at the 12-month and 24-month visits (Supplemental Figure 14A). However, the *H. parainfluenzae-Neisseria* ASV9 network module was positively correlated with salivary pH at the 12- and 24-month visits (Supplemental Figure 14B) and a similar trend existed between salivary pH and the community state types (Supplemental Figure 14D). 
...
```{r FIG 15, fig.cap='Relationships between unsupervised clusters, salivary pH and future S. mutans detection', fig.height=12, fig.width=25}
ph_smut
ggsave(ph_smut, file=file.path(supp.out, 'SupplementalFigure15.pdf'), height=12, width=25)
```
...
```{r TABLE 9}
write.xlsx(t3_table, file=file.path(supp.out, 'AdditionalFile8.xlsx'), overwrite=T)
```

```{r FIG 17, fig.cap='Volcano plot taxa differentially abundant between cases and controls in whole genome shotgun sequenced plaque and saliva samples', fig.width=8, fig.height=11}
taxa_volc
ggsave(taxa_volc, filename=file.path(supp.out, 'SupplementalFigure17.pdf'), units=c('in'), width=18, height=8)
```

```{r Table 10}
r3_table=r3%>%select(SampleType, KEGG, kname, baseMean, log2FoldChange, lfcSE, stat, pvalue, padj, PathAnnotation)%>%arrange(padj)
write.xlsx(r3_table, file=file.path(supp.out, 'AdditionalFile9.xlsx'), overwrite = T)
```

Additional file 8 contains the results from the DESEQ2 analysis on the taxa from WGS sequenced plaque and saliva samples, including the log2 fold change and adjusted *P* values. Supplemental figure 16 presents the same information as a volcano plot. Additional file 9 contains the results from the DESEQ2 analysis of KEGG orthologs from WGS sequenced plaque and saliva samples while additional file 10 contains the result from GSEA (gene set enrichment) analysis if the KEGG pathways. 

Functional differences sometimes reflect taxonomic differences. For example, all the case enriched KEGG orthologs annotating to oxidative phosphorylation were found only in the genus Candida. For the 18 KEGG orthologs which were significantly different between cases and controls at a Benjamini-Hochberg corrected alpha of 0.05 and annotated to oxidative phosphorylation as their first KEGG pathway, additional file 11 shows the KEGG ortholog number, KEGG ortholog name, log2 fold change and all the taxonomic assignments for the contiguous sequences in which that KEGG ortholog was found. All the KEGG orthologs more abundant in cases were found only in either yeast or unclassified contiguous contigs, while the KEGG ortholgos more abundant in controls were found in bacterial species. Supplemental figure 17 shows the abundance of the species in which the KEGG orthologs were found.
...
```{r FIG 18, fig.cap='Abundance of taxa responsible for oxidative phosphorylation KEGG orthologs differentially abundant between cases and controls in plaque samples', fig.height=8, fig.width=10}
ggsave(op_plot, filename=file.path(supp.out, 'SupplementalFigure18.pdf'), units=c('in'), width=10, height=8)
op_plot
```

```{r Table 12}
write.xlsx(op_table, file=file.path(supp.out, 'AdditionalFile11.xlsx'), overwrite = T)
```
...
write.xlsx(enrichment_table, file.path(supp.out, 'AdditionalFile10.xlsx'), overwrite=T)
