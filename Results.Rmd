...
network_models=hellinger_network$module.data%>%
  ...
  mutate(case_numeric=ifelse(Case=='Case', 1, 0),
         IncidentVisit2f=factor(IncidentVisit2, levels=c(8, 7, 5, 9, 10), 
                               labels=c('36mos', '24mos', '12mos', '48 mos', '60 mos')),
         modAbund100=modAbund*100)

net_models=network_models %>% 
...
net_models%>%
  filter(module %in% c('yellow', 'brown'))%>%
  ungroup()%>%
  unnest(adjusted_tidy)%>%
  select(module_label3, Visit, term, estimate, conf.low, conf.high, p.value)%>%
  filter(p.value<0.05)

make_table=function(x, predictor='net'){
...
my_net_models=net_models %>% 
  filter(module %in% c('brown', 'yellow') & Visit %in% c(5, 7))%>%
  mutate(adjusted_table=map(model_adjusted, ~make_table(.x)), 
         Visit=case_when(Visit==5~'12mos', 
                         Visit==7~'24mos'), 
         label=paste0(module_label2, '\n', Visit))%>%
  arrange(module, Visit)
  

tbl_merge(my_net_models$adjusted_table, 
            tab_spanner=my_net_models$label) 
...
my_cst_models %>%
  ungroup()%>%
  unnest(adjusted_tidy)%>%
  select( Visit, term, estimate, conf.low, conf.high, p.value)%>%
  filter(p.value<0.05)

my_cst_models=my_cst_models %>% 
  filter(Visit %in% c(5, 7))%>%
  mutate(adjusted_table=map(model_adjusted, ~make_table(.x, predictor='cst')), 
         label=case_when(Visit==5~'12mos', 
                         Visit==7~'24mos'))
tbl_merge(my_cst_models$adjusted_table, 
            tab_spanner=my_cst_models$label) 
```
...
netcst=left_join(left_join(hellinger_network$module.data, 
                  ...
                  left_join(div%>%
                            dplyr::select(FoxCavID, Chao1, Shannon, Abundance_Streptococcus_mutans,Prevalence_Streptococcus_mutans))%>%
...
ggsave(ph_smut, file=file.path(main.out, 'Figure4.pdf'), height=12, width=25)
...
taxa_of_interest=c('Neisseria perflava', 'Neisseria mucosa', 'Haemophilus parainfluenzae', 'Fusobacterium periodonticum', 'Fusobacterium nucleatum', 'Streptococcus sangunis', 'Prevotella salivae', 'Actinomyces graevenitzii', 'Streptococcus vestibularis', 'Prevotella histicola', 'Streptococcus sobrinus',
                   ...
                   'Veillonella dispar', 
                   'Veillonella parvula')
t3=t3%>%mutate(label=ifelse(Taxa %in% taxa_of_interest & padj<0.05, Taxa, ifelse((-log10(pvalue)>10)|(-log10(pvalue)>10 & SampleType=='saliva') | (SampleType=='saliva' & log2FoldChange < -2), Taxa, '')))
...
ggsave(taxa_volc, filename=file.path(main.out, 'Figure5.pdf'), units=c('in'), width=18, height=8)
...
ggsave(fig4, filename=file.path(main.out, 'Figure6.pdf'), units=c('in'), width=18, height=8)
ggsave(fig4_opt2, filename=file.path(main.out, 'Figure6_opt2.pdf'), units=c('in'), width=18, height=10)

cairo_ps(file.path(main.out, 'Figure6.eps'), family='Arial', width=18, height=10)
fig4
dev.off()

cairo_ps(file.path(main.out, 'Figure6_opt2.eps'), family='Arial', width=18, height=10)
fig4_opt2
dev.off()
